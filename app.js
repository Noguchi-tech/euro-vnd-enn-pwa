...
function appendDoubleZero() {
  if (!state.amountInput || state.amountInput === "0" || state.amountInput === "-0") {
    state.amountInput = "0";
    return;
  }

  if (state.amountInput.replace("-", "").replace(".", "").length + 2 > MAX_INPUT_LENGTH) {
    return;
  }

  state.amountInput += "00";
}
...
function render() {
  ...
  elements.expressionDisplay.textContent = state.calculatorExpression || "計算式なし";
}
